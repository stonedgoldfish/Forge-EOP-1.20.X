...
public class EOPGeckoRenderLayer extends AbstractPackRenderLayer {
    ...
    public final List<ParsedAnimationController<EOPGeckoLayerState>> animationControllers;
    ...
    public EOPGeckoRenderLayer(SkinTypedValue<DynamicTexture> texture, SkinTypedValue<DynamicTexture> modelLocation, ResourceLocation animationLocation, List<ParsedAnimationController<EOPGeckoLayerState>> animationControllers, RenderTypeFunction renderType, DynamicColor tint) {
        ...
        this.tint = tint;
        this.model = new EOPGeckoRenderLayerModel(this);
    }

    public EOPGeckoRenderLayerModel getModel() {
        return model;
    }

    @Nullable
    public EOPGeckoLayerState getState(LivingEntity entity) {
        if (entity instanceof PalladiumLivingEntityExtension extension) {
            return extension.palladium$getRenderLayerStates().getOrCreate(this) instanceof EOPGeckoLayerState state ? state : null;
        }
        return null;
    }
    ...
    public RenderLayerStates.State createState() {
        return new EOPGeckoLayerState(this);
    }
    ...
    public void renderArm(DataContext context, HumanoidArm arm, PlayerRenderer playerRenderer, PoseStack poseStack, MultiBufferSource bufferSource, int packedLight) {
        ...
        if (living != null && IPackRenderLayer.conditionsFulfilled(living, this.conditions, this.firstPersonConditions)) {
            ...
            if (state != null && bone != null) {
                ...
                AnimationState<EOPGeckoLayerState> animationState = new AnimationState<>(state, 0, 0, partialTick, false);
    ...
    public static EOPGeckoRenderLayer parse(JsonObject json) {
        ...
        layer.model.rightHeldItemBone = GsonHelper.getAsString(bonesJson, "right_held_item", layer.model.rightHeldItemBone);
        layer.model.leftHeldItemBone = GsonHelper.getAsString(bonesJson, "left_held_item", layer.model.leftHeldItemBone);
